...
def maps_json():
    ...
    """
    # quick fix to pick default map. 
    ms = {k: v for k, v in get_mapsources().items()}
    if "default" not in ms:
        default = sorted(ms.keys())[0]
        ms["default"] = ms[default]
        del ms[default]
...
def map_to_pdf(map_source, zoom, x, y, width, height):
    ...
    """
    map_source = get_mapsources()[map_source]
...
def kml_master():
    ...
    kml_doc = KMLMaster(app.config["url_formatter"], get_mapsources().values())
